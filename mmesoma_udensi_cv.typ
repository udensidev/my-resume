...
#let design-text-font-size = 7.5pt
...
#block(
  [
    #set par(spacing: 0pt)
    #v(design-highlights-top-margin);#highlights([GPA: 4.0\/4.0],[Coursework: Big Data Analytics, Enterprise Database Management, Big Data Infrastructure, Financial Risk Management],)
  ],
  inset: (
    left: design-entries-left-and-right-margin,
    right: design-entries-left-and-right-margin,
...
#block(
  [
    #set par(spacing: 0pt)
    #v(design-highlights-top-margin);#highlights([GPA: 4.38\/5.0, ranked top 5\% of class],)
  ],
  inset: (
    left: design-entries-left-and-right-margin,
    right: design-entries-left-and-right-margin,
...
#one-col-entry(
  content: [
    #v(design-highlights-top-margin);#highlights([Led the design and implementation of a web-based ETL pipeline to extract, transform, and structure university support service data into CTDL-compliant JSON-LD and downloadable Excel formats],[Engineered data extraction logic to scrape and parse unstructured web content from .edu domains, automating the mapping of over 60 unique services \(e.g., academic advising, disability support\) into a standardized schema],[Defined and validated JSON-LD outputs against Credential Engine's CTDL ontology, ensuring schema integrity across datasets],[Developed a user-facing form interface and backend logic for scalable data ingestion; enabled non-technical users to generate structured outputs without coding],[Authored detailed technical documentation and a user guide, reducing onboarding time for future developers and institutional partners],)
  ],
)
...
#emph[Illinois Business Consulting \(IBC\)]
  ],
  right-content: [
    #emph[Champaign, IL]
...
#one-col-entry(
  content: [
    #v(design-highlights-top-margin);#highlights([Researched and analyzed the impact of emerging AI-driven energy demand trends on grid operations and planning, supporting strategic recommendations for a top consulting firm],[Synthesized technical insights on grid interconnection processes, regulatory changes, and infrastructure capacity into business-ready deliverables for both technical and non-technical stakeholders],[Designed an expert interview guide with over 15 targeted questions, gathering qualitative insights from professionals across the utility, energy policy, and data center ecosystems],[Developed a client-facing presentation deck that distilled over 20 hours of expert interviews and secondary research into executive-ready findings],)
  ],
)
...
#one-col-entry(
  content: [
    #v(design-highlights-top-margin);#highlights([Contributed to the development of a computer vision model to predict crop health],[Annotated a dataset of 10,000+ images to improve the robustness of training data, increasing model accuracy by 10\%],)
  ],
)
...
#two-col-entry(
  left-content: [
    #link("https://github.com/udensidev")[#strong[Yelp Restaurant Business Intelligence]]
  ],
  right-content: [
    #emph[Oct 2024]
  ],
...
#one-col-entry(
  content: [#strong[Tech Stack:] AWS, Bash, Git, Knime, MongoDB, Postman, Python, R, SQL, SAS, Tableau]
)
#v(design-entries-vertical-space-between-entries)
#one-col-entry(
  content: [#strong[Certifications:] Google Data Analytics Professional Certificate, AWS Certified Cloud Practitioner]
)
